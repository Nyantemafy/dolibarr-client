...
CREATE TABLE payement_act (
    ...
    id_presence_act INTEGER REFERENCES presence_act(id_presence_act) ON DELETE CASCADE,  -- Référence à la table activites avec suppression en cascade
...
CREATE VIEW payement_activite AS
    ...
    FROM payement_act
    JOIN presence_act
    ON payement_act.id_presence_act = presence_act.id_presence_act
    JOIN activites
    ON presence_act.id_act = activites.id_act;
...
CREATE VIEW recette_activite AS
    SELECT id_act, COALESCE(SUM(montant), 0) AS total
    FROM payement_act
    JOIN presence_act
    ON payement_act.id_presence_act = presence_act.id_presence_act
    GROUP BY id_act;

CREATE VIEW montant_activite AS
    SELECT 
        presence_act.*, 
        payement_act.id_payement_act,
        payement_act.daty,
